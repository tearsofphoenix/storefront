...
const BACKEND_URL = readRuntimeEnv("MEDUSA_BACKEND_URL")
...
function getFallbackRegionMap(request: NextRequest) {
  const fallbackRegionMap = new Map<string, HttpTypes.StoreRegion | number>()
  const urlCountryCode = request.nextUrl.pathname.split("/")[1]?.toLowerCase()

  if (urlCountryCode && /^[a-z]{2}$/.test(urlCountryCode)) {
    fallbackRegionMap.set(urlCountryCode, 1)
  }

  if (DEFAULT_REGION) {
    fallbackRegionMap.set(DEFAULT_REGION, 1)
  }
...
export async function middleware(request: NextRequest) {
  ...
  // 已带合法 country code 时，不要再重定向到同一路径；只需补写缓存 cookie。
