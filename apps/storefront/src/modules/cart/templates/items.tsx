import repeat from "@lib/util/repeat"
import { getI18n } from "@lib/i18n/server"
import { HttpTypes } from "@medusajs/types"
import { Heading, Table } from "@medusajs/ui"

import Item from "@modules/cart/components/item"
import SkeletonLineItem from "@modules/skeletons/components/skeleton-line-item"

type ItemsTemplateProps = {
  cart?: HttpTypes.StoreCart
}

const ItemsTemplate = async ({ cart }: ItemsTemplateProps) => {
  const { messages } = await getI18n()
  const items = cart?.items
  return (
    <div>
      <div className="flex items-center pb-5">
        <Heading className="text-[1.75rem] leading-[2.2rem] small:text-[2rem] small:leading-[2.75rem]">
          {messages.common.cart}
        </Heading>
      </div>
      <Table className="w-full">
        <Table.Header className="border-t-0">
          <Table.Row className="border-b border-[var(--pi-border)] text-[11px] font-semibold uppercase tracking-[0.14em] text-grey-50">
            <Table.HeaderCell className="!pl-0">
              {messages.common.item}
            </Table.HeaderCell>
            <Table.HeaderCell></Table.HeaderCell>
            <Table.HeaderCell>{messages.common.quantity}</Table.HeaderCell>
            <Table.HeaderCell className="hidden small:table-cell">
              {messages.common.price}
            </Table.HeaderCell>
            <Table.HeaderCell className="!pr-0 text-right">
              {messages.common.total}
            </Table.HeaderCell>
          </Table.Row>
        </Table.Header>
        <Table.Body>
          {items
            ? items
                .sort((a, b) => {
                  return (a.created_at ?? "") > (b.created_at ?? "") ? -1 : 1
                })
                .map((item) => {
                  return (
                    <Item
                      key={item.id}
                      item={item}
                      currencyCode={cart?.currency_code}
                    />
                  )
                })
            : repeat(5).map((i) => {
                return <SkeletonLineItem key={i} />
              })}
        </Table.Body>
      </Table>
    </div>
  )
}

export default ItemsTemplate
